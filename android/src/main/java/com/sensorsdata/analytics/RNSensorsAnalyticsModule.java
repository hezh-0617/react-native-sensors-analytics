...
public class RNSensorsAnalyticsModule extends ReactContextBaseJavaModule {
     ...
     * title="Button"
     * onPress={()=>
     * RNSensorsAnalyticsModule.saveViewScreen(url, {"$title":"RN主页","$screen_name":"cn.sensorsdata.demo.RNHome"})}>
     * </Button>
     */
    @ReactMethod
    public void saveViewScreen(String url, ReadableMap properties) {
        try {
            RNAgent.saveViewScreen(url, RNUtils.convertToJSONObject(properties));
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(LOGTAG, e.toString() + "");
        }
   ...
   /**
     ...
     */
    @ReactMethod
    public void trackWithLastScreenTrackProperties(String eventName, ReadableMap properties) {
        try {
            ...
            // 合并用户传入的属性
            if (properties != null) {
                JSONObject userProperties = RNUtils.convertToJSONObject(properties);
                for (Iterator<String> it = userProperties.keys(); it.hasNext();) {
                    String key = it.next();
                    mergedProperties.put(key, userProperties.get(key));
                }
            }

            // 调用track方法
            SensorsDataAPI.sharedInstance().track(eventName, RNPropertyManager.mergeProperty(mergedProperties));
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(LOGTAG, e.toString() + "");
        }
